...
worker.addEventListener('message', async evt => {
  if (!modelProm) {
    ...
    const { id, data } = evt.data as { id: number; data: Record<string, WorkerTensor> };
    ...
    worker.postMessage({ id, data: outputs }, Object.values(outputs).map(o => (o.data as ArrayBufferView).buffer));
  }
});