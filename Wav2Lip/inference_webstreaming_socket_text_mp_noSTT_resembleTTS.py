# Takes as input text file, and generates audio using resemble TTS to be used for generating video.
...
def text_input_thread_handler(text_input_from, start_audio_input_thread,
                              last_generated_clip_id_queue, text_file_path):
    ...
    if text_input_from == 'socket':
        ...
        # while True:
        #     print("="*50)
        #     print(list(last_generated_clip_id_queue.queue))
        #     print("="*50)
...
def audio_read_thread_from_callback(kill_audio_thread_callback, fifo_resemble_tts,
    ...
    while True:
        ...
        # print("=" * 50)
        # print(fetched_clip_id, last_generated_clip_id)
        # print("=" * 50)
        ...
        if fetched_clip_id == last_generated_clip_id:
            ...
            # print("=" * 50)
            # print('Kill audio callback input set.')
            # print("=" * 50)
...
def txt2vid_inference(fifo_filename_video, audio_inqueue, kill_audio_input_thread):
...
def stream():
    ...
    kill_audio_thread_callback = multiprocessing.Event()  # set in T5 to know that all text received
    # create threads
    audio_callback_thread = multiprocessing.Process(target=audio_read_thread_from_callback,
    ...
    video_thread = threading.Thread(target=txt2vid_inference,
                                    args=(fifo_filename_video, audio_packet_queue_T2, kill_audio_input_thread))
    logger.info('T2: Video thread launched')
    audio_thread = multiprocessing.Process(target=audio_thread_handler,
